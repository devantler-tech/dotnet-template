...
Recommended local validation before opening a PR (warnings are errors, so a clean build is required). The CI workflow runs only a required-checks aggregator and publishing is handled by the reusable publish pipeline on tags, so run these locally:
...
**Validate before any PR (locally):** `dotnet build` then `dotnet test` — recommended local checks; CI itself only runs a required-checks aggregator, so run these yourself before opening a PR. Workflows → `actionlint`.
